  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-8">
      ...
      <div className="bg-gradient-to-r from-green-50 to-blue-50 p-8 rounded-lg border-2 border-green-200">
        <h2 className="text-3xl font-bold text-gray-800 mb-4">Your Savings Potential</h2>
              ...
              <div className="flex justify-between items-start mb-3">
                <div>
                  <span className={`inline-block px-3 py-1 rounded-full text-xs font-semibold ${
                    rec.action === 'optimal' ? 'bg-gray-200 text-gray-700' :
                    rec.action === 'downgrade' ? 'bg-yellow-100 text-yellow-800' :
                    rec.action === 'switch' ? 'bg-blue-100 text-blue-800' :
                    rec.action === 'credits' ? 'bg-purple-100 text-purple-800' :
              ...
              {rec.targetPlan && (
                <div className="flex items-center gap-4 text-sm">
                  ...
                  <span className="text-green-600 font-semibold">
                    Recommended: {rec.targetVendor} {rec.targetPlan} - ₹{rec.newSpend?.toLocaleString('en-IN')}/mo
                  </span>
                </div>
              )}
